...
function finity.new(isdark, gprojectName, thinProject)
    ...
    self2.tip.Text = "Saber Simulator"
    else
    if syn then
    self2.tip.Text = "Saber Simulator  | Synapse"
    else
    if CALAMARI_PLATFORM then
    self2.tip.Text = "Saber Simulator | Octopus"
    else
    if CALAMARI_WINDOWS or CALAMARI_MACOS then
    self2.tip.Text = "Saber Simulator  | Octopus"
    else
    if KRNL_LOADED then
    self2.tip.Text = "Saber Simulator  | Krnl"
    else
    if is_protosmasher_caller then
    self2.tip.Text = "Saber Simulator  | Proto"
    else
    self2.tip.Text = "Saber Simulator  | Support"
    end
    end
    end
    end
